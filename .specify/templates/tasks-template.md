...
- [ ] T004 [P] Declare/version the record schema before producer or consumer code (Constitution I)
...
Include these whenever the feature produces, filters, or reshapes dataset records. Skip with an
explicit note only when the feature touches no release-path data.

- [ ] TXXX Schema validation over 100% of produced records (Constitution I, V)
- [ ] TXXX Blocking automated PII scan over pipeline output (Constitution IV, V)
- [ ] TXXX Run manifest emits seed, serialized config, code revision, input hashes, record counts,
      and filter accounting by reason (Constitution II, III)
