...
- [X] T042 [US1] Implement `LiteLLMModelClient` in `src/ticket_dataset/model/litellm_client.py` — the only module that reaches a provider — with `response_format` structured output, normalized refusal and transport errors, optional model fallbacks, and the served model id surfaced on every response (FR-027a, research R1)
