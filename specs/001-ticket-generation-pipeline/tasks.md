...
exercised against a fake. Only the real client (T042) reaches a provider, and nothing in `tests/` imports it.
...
Single Python project: source in `src/ticket_dataset_generator/`, tests in `tests/`. Paths are repo-relative and
...
- [X] T001 Create the package skeleton `src/ticket_dataset_generator/` with `schema/`, `config/`, `planning/`, `model/`, `generation/`, `privacy/detectors/`, `run/`, `cli/` subpackages, each with an `__init__.py`
...
- [X] T006 Register the CLI entry point `ticket-dataset-generator = "ticket_dataset_generator.cli.main:app"` under `[project.scripts]` in `pyproject.toml`
...
- [X] T010 [P] Define `Role` (`customer`, `agent`), `Category`, `Priority`, `Channel`, and `ResolutionStatus` closed enumerations in `src/ticket_dataset_generator/schema/enums.py` (FR-005, FR-006)
- [X] T011 [P] Define `PIICategory` (blocking floor `EMAIL`, `PHONE`, `CREDIT_CARD`, `US_SSN`; advisory `IP_ADDRESS`, `POSTAL_CODE`), `DiscardReason` (the nine members FR-026b enumerates), `Verdict`, and `RunOutcome` (`completed`, `refused`, `failed`, `stopped`) in `src/ticket_dataset_generator/run/enums.py` (FR-018, FR-018b, FR-026b, FR-036b)
- [X] T012 Define `SCHEMA_VERSION = "1.0.0"` and semver parsing in `src/ticket_dataset_generator/schema/version.py` (FR-002)
- [X] T013 Define `TicketRecord`, `ConversationTurn`, `TicketMetadata`, `RecordQuality`, and `GenerationInfo` Pydantic models in `src/ticket_dataset_generator/schema/record.py` per [data-model.md](./data-model.md), including the `subdomain` and `scenario` fields and a validator enforcing that `resolved_at` is present exactly when `resolution_status` is `resolved` (FR-003, FR-004, FR-006b, FR-008b, FR-009i)
- [X] T014 Implement `export_json_schema()` in `src/ticket_dataset_generator/schema/export.py`, returning the Pydantic export **normalized for comparison**: `$id` and `title` set from `SCHEMA_VERSION`, `description` keys stripped at every level, and object keys sorted — prose descriptions are documentation, not contract (FR-001)
...
- [X] T017 Define `GenerationConfig`, `Composition`, `ModelSpec`, `Budget`, and `TimeWindow` models in `src/ticket_dataset_generator/config/models.py` per [data-model.md](./data-model.md) (FR-008, FR-009r, FR-012f)
- [X] T018 [P] Define the documented defaults — distribution per dimension, 4–12 turn range, thresholds 0.8 / 10% / 0.5% / ±2pp, concurrency and rate bounds — in `src/ticket_dataset_generator/config/defaults.py`, matching the normative table in FR-033
- [X] T019 Implement `load_config()` in `src/ticket_dataset_generator/config/loader.py` performing **total** validation and raising `ConfigError` carrying *every* problem rather than the first (FR-011)
...
- [X] T021 Implement `slot_random(seed, position, attempt)` in `src/ticket_dataset_generator/planning/seeding.py` using counter-based `blake2b` derivation (FR-012b, research R2)
...
- [X] T023 Implement largest-remainder apportionment in `src/ticket_dataset_generator/planning/apportion.py`, including the achievability precondition `tolerance >= 100 / record_count` and `UnsatisfiableCompositionError` naming the dimension and reason (FR-030, FR-031b, FR-032)
...
- [X] T025 Implement `Slot` construction in `src/ticket_dataset_generator/planning/slots.py`: apportioned assignment, uniform turn count, seeded subdomain, and seeded `created_at`/`resolved_at` from the configured window and duration bounds (FR-006a, FR-008d, FR-009d, FR-012b)
...
- [X] T027 Implement fresh-per-run `run_id` and UUIDv5 `record_id` derivation in `src/ticket_dataset_generator/run/ids.py` (FR-003a, FR-003b)
...
- [X] T029 Define the `ModelClient` protocol, `ModelRole`, and `ModelResponse` (carrying the **served** model id, stop reason, usage, retries) in `src/ticket_dataset_generator/model/client.py` per [contracts/model-io.md](./contracts/model-io.md)
- [X] T030 Implement `FakeModelClient` in `src/ticket_dataset_generator/model/fake.py` with scripted responses covering valid output, malformed JSON, refusal, rate-limit error, and PII-bearing content — the fixture every offline test depends on
- [X] T031 [P] Implement the token-bucket rate limiter in `src/ticket_dataset_generator/model/limiter.py`, shared across both model roles (FR-012e)
...
- [X] T033 [P] Define the exception hierarchy in `src/ticket_dataset_generator/errors.py`: `TicketDatasetError` plus the eight subclasses in [contracts/python-api.md](./contracts/python-api.md), none of which may carry a matched PII value
- [X] T034 [P] Define the `GeneratedConversation` and `JudgeVerdict` wire models in `src/ticket_dataset_generator/model/wire.py`, deliberately excluding provenance fields so the model cannot write them (contracts/model-io.md)
...
- [X] T037 [US1] Implement the domain document parser in `src/ticket_dataset_generator/generation/domain_doc.py`, raising `PromptDocumentError` when no usable subdomain list is declared (FR-008d)
...
- [X] T039 [US1] Implement the rubric parser in `src/ticket_dataset_generator/generation/rubric.py`, refusing a rubric whose weights do not sum to 1 (FR-009p)
- [X] T040 [P] [US1] Add unit tests in `tests/unit/test_rubric.py` covering weight validation and `rubric_id` extraction

### Generation and judging

- [X] T041 [US1] Implement prompt assembly in `src/ticket_dataset_generator/generation/prompts.py` with a **byte-stable system prefix** (domain document, rubric) and per-slot user content (assignment, turn count, language, subdomain), so the prefix caches across a run (contracts/model-io.md)
- [X] T042 [US1] Implement `LiteLLMModelClient` in `src/ticket_dataset_generator/model/litellm_client.py` — the only module that reaches a provider — with `response_format` structured output, normalized refusal and transport errors, optional model fallbacks, and the served model id surfaced on every response (FR-027a, research R1)
  - **Revised 2026-08-20.** First implemented as `AnthropicModelClient` against the vendor SDK. No functional requirement names a vendor, and research R1 had never weighed a multi-vendor option, so provider access moved to litellm. The `ModelClient` seam meant the change touched one module, its documentation, and the model settings in the configuration — nothing in the pipeline, the gates, or the record and manifest contracts.
- [X] T043 [US1] Implement generation and structural validation in `src/ticket_dataset_generator/generation/generator.py`: parse, validate against `GeneratedConversation`, and check turn count, customer-first alternation, and non-empty content, discarding under the named reason rather than coercing (FR-009, FR-009b, FR-009d)
...
- [X] T045 [US1] Implement coherence judging in `src/ticket_dataset_generator/generation/judge.py`, computing the score as the **weighted mean of per-criterion scores** using the rubric's declared weights rather than any holistic number the model returns (FR-009f, FR-009p)
...
- [X] T047 [US1] Implement the concurrent pipeline in `src/ticket_dataset_generator/generation/pipeline.py`: bounded `asyncio` worker pool, slot-level retry with the single attempts knob, and the consecutive-failure circuit breaker (FR-012a, FR-009o, FR-012d, spec Edge Cases)

### Ordered output

- [X] T048 [US1] Implement the ordered writer in `src/ticket_dataset_generator/run/writer.py`: a reorder buffer bounded by `max_concurrency`, ascending-position writes with deterministic serialization, and the destination claim at run start. Output stops at the staging file under `data/interim/<run_id>/`; the writer MUST NOT expose a release-path move yet (FR-012, FR-012c, FR-014, FR-014a, research R5)
...
- [X] T050 [P] [US1] Implement turn-sequence fingerprinting and duplicate counting in `src/ticket_dataset_generator/dedup.py`, Unicode-normalized, metadata excluded, reported never discarded (FR-034, FR-039, research R13)
...
- [X] T052 [US1] Implement `GenerationRun.execute()` in `src/ticket_dataset_generator/run/run.py` wiring config validation → slot planning → concurrent generation → judging → schema validation → ordered write (FR-007)
- [X] T053 [US1] Implement the `generate` command in `src/ticket_dataset_generator/cli/main.py` with `--config`, `--seed`, `--out`, `--dry-run`, `--quiet`, progress on stderr, and the four exit statuses (contracts/cli.md, FR-036b)
...
- [X] T059 [P] [US2] Implement the offline `datafog` detector wrapper in `src/ticket_dataset_generator/privacy/detectors/datafog.py`, forcing `DATAFOG_TELEMETRY=0` explicitly rather than trusting the upstream default (FR-024, research R7)
- [X] T060 [P] [US2] Add committed synthetic canary values — one per floor type — in `tests/fixtures/canaries/` and `src/ticket_dataset_generator/privacy/canaries.py` (FR-018a)
- [X] T061 [US2] Implement `DetectorRegistry` in `src/ticket_dataset_generator/privacy/registry.py` with **demonstrated** floor coverage via canary probes at run start, the advisory/blocking tier, exception suppression, and fail-closed handling of a detector that raises (FR-017, FR-017a, FR-018, FR-018a, FR-018b)
- [X] T062 [US2] Fix the scanned-field set in `src/ticket_dataset_generator/privacy/registry.py`: every conversation turn's `content` and the record's `scenario`, and nothing else — `subdomain`, identifiers, enumerated metadata, and timestamps are excluded by requirement rather than by omission (FR-023a)
...
- [X] T066 [US2] Implement deterministic, irreversible masking in `src/ticket_dataset_generator/privacy/masking.py` — domain for an email, issuer range for a card, shape and length otherwise (FR-020a)
...
- [X] T068 [US2] Implement the fingerprint-based `ExceptionStore` in `src/ticket_dataset_generator/privacy/exceptions_store.py`, recording approver and date, **running the detectors over the stated reason** and refusing one that trips them (FR-022, FR-022a, FR-022b, research R9)
...
- [X] T070 [US2] Implement the quarantine writer in `src/ticket_dataset_generator/privacy/quarantine.py`, appending privacy-discarded records under `data/interim/<run_id>/` and never to the release path (FR-021b)
- [X] T071 [US2] Wire the scan into `src/ticket_dataset_generator/generation/pipeline.py` **before the judge call**, so every structurally valid response is scanned and no judging call is spent on a record about to be discarded for privacy (FR-016, FR-016a, FR-021)
- [X] T072 [US2] Add the atomic staging→release move to `src/ticket_dataset_generator/run/writer.py`, re-verifying the destination claim immediately before it, and guard it so it refuses unless the detector registry passed its floor probe for this run — no code path may place output in `data/release/` that the privacy gate has not examined (FR-014a, FR-015, FR-016, FR-018a, Constitution IV)
- [X] T073 [US2] Implement `privacy scan` and `privacy approve` in `src/ticket_dataset_generator/cli/main.py`, including `--from-quarantine` and `--by`, and a report carrying **how many records and fields were examined**, the field set scanned, the detectors that ran, and covered **and** uncovered identifier types — the counts are what distinguish a clean result from a scan that examined nothing (FR-019, FR-022a, FR-023, FR-023a, contracts/cli.md)
...
- [X] T077 [US3] Implement code-revision capture and input hashing in `src/ticket_dataset_generator/run/revision.py`: git SHA, explicit dirty flag, unavailable reason, `sha256` per input, and environment settings capable of altering routing (FR-008c, FR-025a, research R10)
...
- [X] T079 [US3] Implement manifest construction in `src/ticket_dataset_generator/run/manifest.py`, written beside the artifact as `<run_id>.manifest.json`, carrying start/end times, both model identities, fallback tallies, per-segment code revisions, budget spend, output filename and checksum (FR-025, FR-025b, FR-027, FR-029a, FR-015f)
- [X] T080 [US3] Implement `validate_manifest()` in `src/ticket_dataset_generator/run/manifest.py` checking contract conformance **and** the reconciliation `records_generated - discards == records_written`, naming every discrepancy (FR-026, FR-026a, FR-028)
...
- [X] T083 [US3] Implement `RunReport` in `src/ticket_dataset_generator/run/report.py` — one object driving the JSON output, the human rendering, the `RunOutcome`, and the exit status, with the score histogram in fixed 0.05 buckets (FR-035, FR-036, FR-036a, FR-036b, FR-038, research R9)
- [X] T084 [US3] Implement mid-run threshold evaluation in `src/ticket_dataset_generator/run/thresholds.py`, firing once `max(1000, 5% of record_count)` records exist and stopping-and-checkpointing on breach; composition stays end-only (FR-037, FR-037a)
...
- [X] T086 [US3] Implement checkpointing in `src/ticket_dataset_generator/run/checkpoint.py`: periodic durable writes, `bytes_written` truncation resume, input-fingerprint comparison, candidate discovery, and `CheckpointCorruptError` preserving partial output (FR-015a, FR-015e, FR-015g, FR-015h, research R6)
...
- [X] T088 [P] [US3] Implement run budgets in `src/ticket_dataset_generator/run/budget.py` — wall-clock and model-call ceilings that stop and checkpoint rather than fail (FR-012f)
- [X] T089 [P] [US3] Implement retention in `src/ticket_dataset_generator/run/retention.py`: drop staging and checkpoint on success, keep report and quarantine, keep everything on failure or interruption (FR-015i)
- [X] T090 [US3] Implement `GenerationRun.resume()` in `src/ticket_dataset_generator/run/run.py`, adding a manifest segment per resume and reconciling tallies into one manifest (FR-015b, FR-015c, FR-015d, FR-015f)
- [X] T091 [US3] Implement `validate-manifest` in `src/ticket_dataset_generator/cli/main.py` (contracts/cli.md)
...
- [X] T096 [US4] Implement per-member tolerance evaluation in `src/ticket_dataset_generator/planning/tolerance.py`, returning one `Breach` per offending member with dimension, member, requested, achieved, and drift (FR-031)
...
- [X] T098 [US4] Record all three distributions — requested, assigned, achieved — in `src/ticket_dataset_generator/run/manifest.py` and `src/ticket_dataset_generator/run/report.py`, so apportionment error and discard-induced drift are distinguishable (FR-031a)
- [X] T099 [US4] Wire composition refusals into `load_config()` in `src/ticket_dataset_generator/config/loader.py`: proportions not summing, unknown member, a proportion too small to round, and a tolerance unachievable at the corpus size (FR-031b, FR-032)
...
- [X] T102 [P] Implement `sample-for-review` in `src/ticket_dataset_generator/cli/main.py` exporting a seeded random sample with scores, so SC-011 calibration is cheap (contracts/cli.md)
- [X] T103 [P] Implement `schema export` in `src/ticket_dataset_generator/cli/main.py`, writing the normalized export to stdout or `--out` (Constitution I). The drift check itself is T015, already running under the workflow added in Phase 1
- [X] T104 [P] Re-export the public surface from `src/ticket_dataset_generator/__init__.py` exactly as [contracts/python-api.md](./contracts/python-api.md) specifies
...
- [X] T115 Verify every data-transforming module under `src/ticket_dataset_generator/` has contract and edge-case tests under `tests/` before it produces a released artifact (Constitution V)
  - **Verified 2026-08-20.** 43 modules; the check found 10 without direct tests and five of them warranted new suites — `privacy/fiction.py`, `generation/frontmatter.py`, `run/budget.py`, `run/retention.py`, `run/report.py`, plus `generation/prompts.py` and `generation/pipeline.py`. Writing them found two defects: the reserved-domain check matched only apexes, so `sub.example.com` was not recognised as reserved, and the report's worst-drift tie-break depended on set order, so an equally-drifted pair could be named differently on different runs. Two modules remain without direct tests, both deliberately: `config/defaults.py` holds constants and transforms nothing, and `model/litellm_client.py` is the network boundary that no test may exercise.
- [X] T116 Provide the means to record a documented random-sample human review and the calibration behind it, so whoever publishes a corpus can discharge Constitution V and SC-011
  - **Rescoped 2026-08-24.** As written this task said "record ... before any release", which is an act this repository never performs: it is a general-purpose generator, and corpora are published manually elsewhere. A task gating a release that never happens here could never be closed. What the repository owes is the means — `sample-for-review` draws a reproducible sample, `calibrate` compares it against the judge on gate agreement and rank correlation, and `calibration/` holds one committed record per rubric version. Exercised end to end: `calibration/coherence-v2.calibration.json`. Checklist CHK063 still stands — nothing obliges a calibration to exist, and a release is not blocked by its absence.
- [X] T117 Provide a dataset card template and a stated versioning convention covering composition, generation method, model identity, known limitations, intended use, and every active privacy exception (Constitution: Development Workflow; FR-022a)
  - **Rescoped 2026-08-24**, for the same reason. `datasheets/TEMPLATE.md` is domain-agnostic and marks every value that must be read from an artifact rather than composed, with the field it comes from; `datasheets/EXAMPLE.md` is a filled-in instance; `datasheets/README.md` states the `MAJOR.MINOR.PATCH` convention, the `dataset-v<version>` tag, and the `cc-by-4.0` default. Bumping a version and filling the card in are per-corpus acts belonging to whoever publishes one.
