...
Single Python project: source in `src/ticket_dataset_generator/`, tests in `tests/`. Paths below are repo-relative
...
- [ ] T001 Create the package skeleton `src/ticket_dataset_generator/` with `schema/`, `io/`, `validation/`, `invariants/`, `privacy/detectors/`, `manifest/`, `report/`, `gate/`, `cli/` subpackages, each with an `__init__.py`
...
- [ ] T006 Register the CLI entry point `ticket-dataset-generator = "ticket_dataset_generator.cli.main:app"` under `[project.scripts]` in `pyproject.toml`
...
- [ ] T009 [P] Define `Role`, `Category`, `Priority`, `Channel`, and `ResolutionStatus` closed enumerations in `src/ticket_dataset_generator/schema/enums.py` per data-model.md
- [ ] T010 [P] Define `PIICategory` (blocking floor `EMAIL`, `PHONE`, `CREDIT_CARD`, `GOVERNMENT_ID`; advisory `IP_ADDRESS`, `POSTAL_CODE`), `Verdict`, and `GateName` enumerations in `src/ticket_dataset_generator/report/enums.py` (FR-013d, FR-013f)
- [ ] T011 Define `SCHEMA_VERSION = "1.0.0"`, semver parsing, and `is_supported_version()` enforcing the exactly-one-version rule in `src/ticket_dataset_generator/schema/version.py` (FR-002a, FR-002b)
- [ ] T012 Define the `TicketRecord`, `ConversationTurn`, and `TicketMetadata` Pydantic models in `src/ticket_dataset_generator/schema/record.py`, including the FR-006c cross-field validators (`resolved_at` not before `created_at`; `resolved_at` required when `resolution_status` is `resolved`; turn timestamps within ticket bounds)
- [ ] T013 Implement `export_json_schema()` and a `--check` drift comparison against `specs/001-ticket-generation-pipeline/contracts/record.schema.json` in `src/ticket_dataset_generator/schema/export.py` (R1, Principle I)
- [ ] T014 [P] Implement the streaming line-by-line JSONL reader yielding `ParsedLine(line_no, parsed | error)` in `src/ticket_dataset_generator/io/jsonl.py`, never loading the whole file and continuing past unparseable lines (FR-008, FR-012)
- [ ] T015 [P] Define the `Report`, `Finding`, and `PrivacyException` models in `src/ticket_dataset_generator/report/model.py`, including `declared_gaps`, `detectors_run`, `records_examined`, and `counts_by_gate`, and enforcing that `Finding` has no field capable of holding a matched PII value (data-model.md, R4)
- [ ] T016 Implement `render_json()`, `render_text()`, and `exit_status()` over the single `Report` object in `src/ticket_dataset_generator/report/render.py`, so JSON, text, and exit status cannot disagree (R9, FR-011, FR-031)
...
- [ ] T019 Re-export the public API surface from `src/ticket_dataset_generator/__init__.py` exactly as listed in [contracts/python-api.md](./contracts/python-api.md) (FR-029)
- [ ] T020 [P] Add a contract test in `tests/contract/test_public_api.py` asserting every name in [contracts/python-api.md](./contracts/python-api.md) is importable from `ticket_dataset_generator` with the documented signature
...
- [ ] T025 [US1] Implement `validate_records()` in `src/ticket_dataset_generator/validation/validator.py`: stream via `read_jsonl`, validate each record against `TicketRecord`, and emit one `Finding` per failure naming the field and reason (FR-007, FR-008, FR-009)
- [ ] T026 [US1] Report unparseable lines by line number with `record_id = None` and continue the run rather than stopping at the first error, in `src/ticket_dataset_generator/validation/validator.py` (FR-009, spec Edge Cases)
- [ ] T027 [US1] Implement unsupported-version detection as a distinct outcome and mixed-version file rejection naming each version present, in `src/ticket_dataset_generator/validation/validator.py` (FR-010, FR-010a)
...
- [ ] T029 [US1] Add the `validate` command to `src/ticket_dataset_generator/cli/main.py`, wiring arguments to `validate_records` and returning `exit_status(report)` with no checking logic of its own (FR-030, FR-032)
...
- [ ] T037 [P] [US2] Define the `Detector` protocol (`name`, `categories`, `scan`) in `src/ticket_dataset_generator/privacy/base.py` (FR-013a)
- [ ] T038 [US2] Implement `DetectorRegistry` with `register`, `covered_categories`, `assert_floor_covered` (failing closed on an uncovered blocking floor), and `scan_text` in `src/ticket_dataset_generator/privacy/registry.py` (FR-013a, FR-013d)
- [ ] T039 [US2] Implement the datafog detector in `src/ticket_dataset_generator/privacy/detectors/datafog_detector.py`, setting `DATAFOG_TELEMETRY=0` before construction and mapping datafog entity names to `PIICategory` (R2, FR-013c)
- [ ] T040 [US2] Implement fingerprint-based exceptions — `fingerprint(category, value)` as `sha256(category + ":" + normalized)` and `load_exceptions()` — in `src/ticket_dataset_generator/privacy/exceptions.py`, never persisting raw values (R4, FR-016)
...
- [ ] T042 [US2] Implement `scan_privacy()` in `src/ticket_dataset_generator/privacy/scan.py`: scan every text field of every record, populate `detectors_run`, and report the examined record and field counts (FR-013, FR-013b, FR-017)
...
- [ ] T044 [US2] Mark non-floor categories as advisory, non-blocking findings that appear in the report but never fail the gate, in `src/ticket_dataset_generator/privacy/scan.py` (FR-013f)
- [ ] T045 [US2] Add the `scan` command with `--exceptions` to `src/ticket_dataset_generator/cli/main.py`, returning exit status `3` distinctly when floor coverage fails (FR-030, contracts/cli.md)
...
- [ ] T050 [P] [US3] Implement turn-ordering, role-alternation, and empty/whitespace-only turn checks in `src/ticket_dataset_generator/invariants/checks.py` (FR-018)
- [ ] T051 [US3] Implement exact-duplicate detection in `src/ticket_dataset_generator/invariants/dedup.py` by fingerprinting the turn sequence only — role plus whitespace-normalized content — excluding provenance and metadata (FR-018a, R6)
- [ ] T052 [US3] Group duplicates and identify the retained record in the report output from `src/ticket_dataset_generator/invariants/dedup.py` (FR-019)
- [ ] T053 [US3] Report duplicate record IDs as a violation distinct from duplicate content in `src/ticket_dataset_generator/invariants/dedup.py` (FR-020)
- [ ] T054 [US3] Implement `check_invariants()` in `src/ticket_dataset_generator/invariants/run.py` reporting all violations across a file in one pass, holding only digests and IDs in memory (FR-021, R5)
- [ ] T055 [US3] Add the `invariants` command to `src/ticket_dataset_generator/cli/main.py` (FR-030)
...
- [ ] T060 [P] [US4] Define the `RunManifest`, `CodeRevision`, and `RemovalAccount` models in `src/ticket_dataset_generator/manifest/model.py`, enforcing `input_count - sum(removals) == output_count` (FR-022, FR-023)
- [ ] T061 [US4] Implement `capture_code_revision()` in `src/ticket_dataset_generator/manifest/capture.py` recording the git SHA plus an explicit dirty-tree flag, or a stated reason when unavailable (R8)
- [ ] T062 [P] [US4] Implement `hash_input()` as sha256 over file contents in `src/ticket_dataset_generator/manifest/capture.py` (FR-022)
- [ ] T063 [US4] Implement `write_manifest()` and `validate_manifest()` naming any missing required element in `src/ticket_dataset_generator/manifest/io.py` (FR-024)
- [ ] T064 [US4] Add the `manifest-check` command to `src/ticket_dataset_generator/cli/main.py` (FR-030)
...
- [ ] T065 Implement `run_gate()` in `src/ticket_dataset_generator/gate/run.py` running schema → privacy → invariants (plus manifest validation when given), continuing past a failed gate so the full picture appears in one pass, and consolidating into a single `Report` (FR-026, FR-028, spec Edge Cases)
- [ ] T066 Treat an empty artifact as a gate failure rather than a trivial pass in `src/ticket_dataset_generator/gate/run.py` (FR-027)
- [ ] T067 Add the `gate` and `export-schema` commands to `src/ticket_dataset_generator/cli/main.py` (contracts/cli.md)
...
- [ ] T079 [P] Verify contract and edge-case tests exist for every data-transforming module by auditing coverage of `src/ticket_dataset_generator/` against `tests/contract/` (Constitution V)
...
T009  Define record enums in src/ticket_dataset_generator/schema/enums.py
T010  Define report enums in src/ticket_dataset_generator/report/enums.py
T014  Streaming JSONL reader in src/ticket_dataset_generator/io/jsonl.py
T015  Report/Finding models in src/ticket_dataset_generator/report/model.py
...
are confined to `src/ticket_dataset_generator/cli/main.py`, where each story appends one command.
