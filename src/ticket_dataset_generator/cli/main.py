...
from ticket_dataset_generator.run.progress import ProgressReporter

app = typer.Typer(
    add_completion=False,
    help="Generate reproducible multi-turn support ticket datasets in any domain.",
)
        ...
        if from_manifest is not None:
            ...
            if seed is not None:
                raise TicketDatasetError(
                    ...
                    "ticket-dataset-generator config-from-manifest <manifest> --out <config>"
                )
...
def _report_reproduction(report) -> None:
    ...
    from ticket_dataset_generator.run.reproduce import InputStatus
...
def _reproduce_from_manifest(
    ...
    """
    from ticket_dataset_generator.run.reproduce import check_reproduction
    ...
    from ticket_dataset_generator.config.loader import validate_config
...
def config_from_manifest_command(
    ...
    if out is not None:
        _note("")
        _note(f"  ticket-dataset-generator generate --config {out} --seed {report.seed}")
        ...
        _json.dumps(
            {
                ...
                # Fill this in, then run `ticket-dataset-generator calibrate`. Left
                # explicitly null rather than absent so a partially scored sample is
                # visible rather than silently measuring a subset.
                #
